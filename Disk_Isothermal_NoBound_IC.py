...
########################################################################
#FUNCTIONS
def diskdens(R): # Disk surface density Sigma
  return Sigma0 * (r_cav/R)**p * np.exp(-(r_cav/R)**2) 
...
def ddiskdens_dRprofile(R):
  return Sigma0 * (r_cav/R)**p * np.exp(-(r_cav/R)**2) * (-p + 2 *(r_cav/R)**2)/R
...
pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R+1.5*thickness,Ntheta=Nphi,
                                BoxSizeX=BoxX,BoxSizeY=BoxY,equal_ids=-2)
pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R+thickness/2.0,Ntheta=Nphi,
                                BoxSizeX=BoxX,BoxSizeY=BoxY,equal_ids=-2)
pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R-thickness/2.0,Ntheta=Nphi,
                            BoxSizeX=BoxX,BoxSizeY=BoxY,equal_ids=-1)
#and add some buffer cells outside the outer boundary
pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R+thickness*2.5,Ntheta=Nphi,
...
R = inner_rad
#thickness = float(2.0*math.pi*R/Nphi)
#pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R+3*thickness/2.0,Ntheta=Nphi,
#                                BoxSizeX=BoxX,BoxSizeY=BoxY)#,equal_ids=-1)
#pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R+thickness/2.0,Ntheta=Nphi,
#                                BoxSizeX=BoxX,BoxSizeY=BoxY)#,equal_ids=-1)
#pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R-thickness/2.0,Ntheta=Nphi,
...
#pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R-thickness,Ntheta=Nphi,
#                            BoxSizeX=BoxX,BoxSizeY=BoxY)#,equal_ids=-3)
#pos,ids = d.add_ring_of_cells(pos=pos,ids=ids,R=R-2*thickness,Ntheta=Nphi,
#                            BoxSizeX=BoxX,BoxSizeY=BoxY)#,equal_ids=-3)
...
print pos_center_x.min(),pos_center_x.max(),inner_rad-0.5*delta,delta
...
print pos.shape, pos_center.shape
...
dens[radius < outer_rad] = vecdiskdens(radius[radius < outer_rad])
#dens[radius >= outer_rad] = Sigma0 * (r_cav/outer_rad)**p * np.exp(-(r_cav/outer_rad)**2) 
R_edge=2.5
dens_cut=vecdiskdens(R_edge)
print "Cut in density=",dens_cut
dens_out = vecdiskdens(outer_rad)
...
if (test_functions):
  ...
  func3 = vecviscosityprofile(x) * x**3 * vecdiskdens(x) * func2
  func4 = np.gradient(func3,np.diff(x).mean())


  func5 = 1/x/ vecdiskdens(x) / (2 * func * x + x**2 * func2) * func4
...
R_edge=2.5
print "Mass resolution inside the cavity",np.pi*(R_edge*2*np.pi/Nphi_in)**2*vecdiskdens(R_edge)
